...
> [!NOTE]
> Para visualizar o diagrama abaixo no VS Code, é necessário ter instalada a extensão [**Mermaid Chart extension for Visual Studio Code**](vscode:extension/MermaidChart.vscode-mermaid-chart) ou similar. No GitHub, a visualização é nativa.
...
erDiagram
    ...
    products ||--o{ transaction_products : "is_part_of"
...
> **Nota:** O registro em `clients` é criado ou atualizado no momento em que uma compra é realizada.
...
| `clients.email` (UNIQUE)                | Garante que cada cliente seja identificado unicamente pelo e-mail                     |
